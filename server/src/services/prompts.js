...
E. RESTATED / CONFLICTING TERMS — NIT, ITB, GCC, SCC and buyer ATC frequently restate the same term with DIFFERENT numbers (e.g. GCC says PBG 5%, SCC says 3%). Do NOT silently pick one or merge them. Extract EACH occurrence as its own item, and set clauseStatus='CONFLICT' with 'conflictNote' describing the discrepancy and the other clauseRef/page it disagrees with.
   CONFLICT means the numbers/values genuinely DISAGREE — it is not a label for "this term is also mentioned elsewhere." Before setting clauseStatus='CONFLICT', check: do the two occurrences state DIFFERENT numbers/values? If they match, this is NOT a conflict — use clauseStatus='EXPLICIT' instead, and if the cross-reference is worth noting, say so briefly inside requirementDescription (e.g. "also stated identically in Section 7 p.99"), never in conflictNote. Never write clauseStatus='CONFLICT' together with a conflictNote that says "no conflict" / "consistent" / "not a conflict" — that combination is always wrong; if your note says there's no discrepancy, the status must be 'EXPLICIT', not 'CONFLICT'.
